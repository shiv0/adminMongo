...
$check=mysqli_query($conn,"SELECT city_name FROM city WHERE city_name='$cityname'");

$count=mysqli_num_rows($check);

if($count>0)
{
 echo '<script>alert("City is Already Register");</script>';
}
else{
 $sql = "INSERT INTO city (city_name,city_status) VALUES ('$cityname','0')";
            if (mysqli_query($conn, $sql)) {
                echo '<script>alert("City Register Successfully");</script>';
            }

else{
   echo '<script>alert("City Registeration Failed");</script>';
}
}
...
?>
 ...
 <div class="row" style="width:900px;">
                    ...
                    <input type="text" class="form-control" id="exampleInputEmail1" name="cityname" placeholder="Enter City Name" required>
                  </div>
